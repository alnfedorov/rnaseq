...
include { PREMAP_STAR                        } from '../modules/local/premap_star'
...
workflow RNASEQ {
    ...
    if (params.premap_index) {
        PREMAP_STAR(
            ch_filtered_reads,
            Channel.value(file(params.premap_index, checkIfExists: true)),
            Channel.value(file(params.premap_exclude, checkIfExists: true)),
        )
        .premapped_reads
        .set { ch_filtered_reads }

        ch_versions = ch_versions.mix(PREMAP_STAR.out.versions.first())
    }
